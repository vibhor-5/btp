  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Key Conclusions}
        \begin{itemize}
          ...
          \item \textbf{Dedicated 24 Slot Heads}: Resolves inter-hour gradient conflicts across operating store hours (+0.0161 F1 boost).
          \item \textbf{GELU Non-Linearity Bottleneck}: Captures non-linear inventory step-functions, achieving peak \textbf{0.8156 F1} and \textbf{91.37\% Stockout Recall}.
        \end{itemize}
      \end{block}
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Future Scope}
        \begin{itemize}
          \item \textbf{Intraday Streaming Data}: Incorporate real-time point-of-sale transaction streams for dynamic intra-day stockout threshold adjustments.
          \item \textbf{Spatial-Store Hierarchy}: Scale hourly slot projections across regional warehouse clusters for multi-echelon inventory replenishment.
          \item \textbf{Censored Demand Modeling}: Integrate explicit choice estimation to unbias true demand during zero-stock hours.
        \end{itemize}
      \end{exampleblock}
    \end{column}
  \end{columns}
